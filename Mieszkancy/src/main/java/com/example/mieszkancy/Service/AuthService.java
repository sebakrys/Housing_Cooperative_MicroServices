...
public class AuthService {
    ...
    public String getUsernameForKeycloak(String JWT_Token){
        //weryfikacja czy jest user
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", JWT_Token);
        HttpEntity<String> request = new HttpEntity<String>(headers);
        try {
            ResponseEntity re= new RestTemplate().exchange(keycloakAuthAddr+"/usernameKeycloak", HttpMethod.GET, request, String.class);
            System.out.println(re.getStatusCodeValue());
            if(re.getStatusCodeValue()!=200){//jesli nie 200 wycofaj
                return null;
            }else{
